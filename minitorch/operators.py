...
def zipWith(fn: Callable[[float, float], float]) -> Callable[[Iterable[float], Iterable[float]], Iterable[float]]:
    ...
    """
    def map2(ls: Iterable[float], ls2: Iterable[float]) -> Iterable[float]:
        return [fn(x, y) for (x, y) in zip(ls, ls2)]
    return map2
...
def reduce(fn: Callable[[float, float], float], start: float) -> Callable[[Iterable[float]], float]:
    ...
    Returns:         Function that takes a list `ls` of elements         $x_1 \ldots x_n$ and computes the reduction :math:`fn(x_3, fn(x_2,         fn(x_1, x_0)))`    """
